...
import type { Banner, FieldErrors, LoginForm, ProfileForm, RegisterForm, RegistrationSession, Screen } from './auth/types'
...
function App() {
  ...
  const [screen, setScreen] = useState<Screen>(() => {
    try {
      ...
      if (path === '/me') {
        // render my profile page outside the auth flow
        return 'login'
      }
  ...
  useEffect(() => {
    ...
    const loadInterests = async () => {
      try {
        ...
        const normalized = interests.filter((value): value is string => typeof value === 'string' && value.trim().length > 0)
  ...
  const handleVerifySubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!pendingRegistration) {
      setBanner({ tone: 'error', text: 'Thiếu thông tin đăng ký. Vui lòng quay lại màn đăng ký.' })
      return
    }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      await api.post('/auth/send-otp', { email: pendingRegistration.email })
      setOtpDigits(Array.from({ length: otpLength }, () => ''))
  ...
  const handleProfileSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const meResp = await api.get('/users/me', { headers: { Authorization: `Bearer ${storedToken}` } })
      const currentUserId = meResp?.data?.id
      // eslint-disable-next-line no-console
      ...
      await api.put('/users/me/profile', safePayload, {
  ...
  const currentStep = screen === 'login' ? 1 : screen === 'register' ? 1 : screen === 'verify' ? 2 : 3
  const showStepper = screen !== 'login'

  // If path is /me, render the authenticated profile page
  if (pathname === '/me') {
    return <MyProfile />
  }
    ...
    <main className="auth-page">
      ...
      <section className="auth-frame">
        ...
        <section className={`flow-wrap ${screen === 'profile' ? 'flow-wrap-profile' : ''}`}>
          {showStepper && (
            <nav className="stepper" aria-label="Tiến trình đăng ký">
              {[1, 2, 3].map((step) => (
                <div key={step} className={`stepper-node ${step <= currentStep ? 'is-active' : ''}`}>
                  <span>{step}</span>
                </div>
              ))}
            </nav>
